...
export default function AdminPage() {
  ...
  const handleSetPremium = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.message || 'Failed to set premium status');
      }
    } catch (error) {
      console.error('Error setting premium status:', error);
      toast.error('An error occurred while setting premium status');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4">Set Premium Status</h2>
        ...
        <form onSubmit={handleSetPremium} className="space-y-4">
          ...
          <div>
            ...
            >
              {loading ? 'Processing...' : 'Set Premium Status'}
            </button>
          </div>
        </form>
        ...
        <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
          <p className="text-sm text-yellow-700 dark:text-yellow-300">
            <strong>Note:</strong> This tool directly modifies user data in Firebase. Use with caution.
          </p>
        </div>
      </div>
